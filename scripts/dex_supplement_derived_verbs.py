...
TOP_N_BY_FREQUENCY: Optional[int] = 1000
...
def _fast_get_html(candidate: str) -> Optional[str]:
    ...
    # Cache miss — return None instead of going to network. The previous
    # full run already cached everything; misses now are nonsense candidates
    # that aren't in DEX, and we'd waste 14s+ on 404 retries per miss.
    return None
